...
**FF-RECON-001 fulfilled (this PR) → Playtesting → Retention Validation → MVP Launch Candidate.**
Feature Flags are now **end-to-end and reconciled onto one core**. Backend (BE-003 / PR #55, merged —
`feature_flags.py` + `balance.yaml` `feature_flags:` + `GET /api/game/flags`, env-overridable,
fail-closed, defaults ON) is the single source of truth, and **this PR consolidates the web layer onto
it** (System A → System B): a runtime `useFlag` hook + `resolveFeature` (web→backend key map,
`cup→cup_competitions` etc.), with `RequireFeature` and a pure `visibleNavLinks` gating the
market/cup/university routes + nav in lock-step (desktop + mobile). The old build-time
`NEXT_PUBLIC_ENABLE_*` path survives only as a fallback for `chain`/`contracts` (no backend flag). A
kill-switch flip now takes effect without a redeploy. **Next chat:** run guided-FTUE playtests and
measure the retention metrics (time-to-first-harvest / first-reward, Day-1 return, replant %,
sessions/day), then iterate FTUE pacing.
