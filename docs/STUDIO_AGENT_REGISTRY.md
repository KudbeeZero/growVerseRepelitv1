...
| BE-003 (Feature Flags — **web gating**) | Builder / DX | A00 | FE-A00, QA-A00 | `claude/web-feature-flag-gating` | (this PR) | **Protected: Navigation + Layout** (Director-approved claim) — `components/layout/{RequireFeature,navLinks,NavBar,MobileTabBar}.tsx`, `lib/features.ts`, new `lib/api/flags.ts` + `hooks/useFlags.ts`; runtime `useFlag` over `GET /api/game/flags`, web→backend key map. No backend / economy / unrelated-UI changes. | backend core (PR #55, merged) | 🟢 Open — Director-approved |
...
| **FF-RECON-001 (Feature Flag Reconciliation)** | Builder / DX | A00 | FE-A00, QA-A00 | `claude/web-feature-flag-gating` | PR #64 | **Protected: Navigation + Layout** + flag layer: web `web/src/lib/features.ts` (System A, build-time `NEXT_PUBLIC_ENABLE_*`) → consume backend `GET /api/game/flags` via a `useFlag` hook; backend `feature_flags.py` + `data/balance.yaml` (`feature_flags:`) as the single registry; `require_feature` as the single gate | Backend core (BE-003 / #55) **merged**; this consolidates A→B | ✅ **FULFILLED by PR #64** (Director-authorized "Feature Flags — web gating" = this reconciliation): runtime `useFlag`/`RequireFeature`/`visibleNavLinks` over `GET /api/game/flags`, web→backend key map, build-time fallback kept only for `chain`/`contracts`. Pending review/merge. |
